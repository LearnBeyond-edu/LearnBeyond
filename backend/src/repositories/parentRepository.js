...
class ParentRepository extends BaseRepository {
  ...
  async findAll(limit = 10, cursor = null, additionalWhere = '', params = []) {
    ...
    if (cursor) {
      whereConditions.push(`pp.id < $${paramIndex}`);
    ...
    queryStr += ` ORDER BY pp.id DESC LIMIT $${paramIndex}`;
  ...
  async delete(id) {
    ...
    if (profile) {
      await query(`DELETE FROM parent_profiles WHERE id = $1`, [id]);
