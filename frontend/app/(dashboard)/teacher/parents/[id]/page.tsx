...
import { useParent, useStudents } from "@/hooks/useSchool";
...
export default function TeacherParentDetailPage() {
  const { id } = useParams<{ id: string }>();
  const { data: parent, isLoading: parentLoading, isError } = useParent(id);
  const { data: studentsData, isLoading: studentsLoading } = useStudents(100);

  const isLoading = parentLoading || studentsLoading;
  ...
  const student = students.find((s: any) => s.user_id === parent?.student_id);

  const studentName = student
    ? [student.first_name, student.last_name].filter(Boolean).join(" ") || `Student ${student.id.slice(0, 6)}`
    : (parent?.student_id ? (studentsLoading ? "Loading child..." : "Student profile missing") : "Unassigned");
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1 grid sm:grid-cols-2 gap-3">
                {[
                  ...
                  { icon: <Heart className="h-4 w-4" />, label: "Student (Child)", value: studentName },
